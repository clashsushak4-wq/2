...
const OrderBookRow = memo(({ price, amount, isAsk }: { price: string; amount: string; isAsk?: boolean }) => {
  ...
  return (
    <div className="flex justify-between items-center relative h-[16px]">
      <div
        className={`absolute right-0 top-0 bottom-0 ${isAsk ? 'bg-zinc-500/40' : 'bg-white/20'}`}
        style={{ width }}
      />
      <span className={`${isAsk ? 'text-zinc-400' : 'text-white'} z-10`}>{price}</span>
...
export const OrderBook = memo(() => {
  ...
  const asks = useMemo(() => {
    ...
    if (amountPercent > 0) {
      baseAsks.unshift({ price: '0.01317', amount: (amountPercent * 1.5).toFixed(2) + 'K' });
    }
    if (isTPSL) {
  ...
  const bids = useMemo(() => {
    ...
    if (amountPercent > 0) {
      baseBids.push({ price: '0.01306', amount: (amountPercent * 1.2).toFixed(2) + 'K' });
    }
    if (isTPSL) {
